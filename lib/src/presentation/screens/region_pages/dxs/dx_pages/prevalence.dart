...
class PrevalencePage extends StatelessWidget {
  ...
  Widget _buildPrevalenceContent(BuildContext context, dynamic orthoData) {
    ...
    if (kDebugMode) {
      print('DEBUG: PrevalencePage - Building content for region: $regionName, disease: $diseaseName');
    }

    // Get prevalence data for the specific disease
    final prevalenceData = DataHelper.getPrevalenceData(
      orthoData,
      regionName,
      diseaseName,
    ) ?? [];
    ...
    if (kDebugMode) {
      print('DEBUG: PrevalencePage - Received prevalence data: $prevalenceData');
    }
    if (kDebugMode) {
      print('DEBUG: PrevalencePage - Data is empty: ${prevalenceData.isEmpty}');
    }
    if (kDebugMode) {
      print('DEBUG: PrevalencePage - Rendering with data length: ${prevalenceData.length}');
    }
